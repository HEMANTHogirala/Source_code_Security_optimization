...
def get_analyze_code_function(language):
    try:
        if language == 'python':
            from python import evaluate_code
        elif language == 'php':
            from php import evaluate_code
        elif language == 'javascript':
            from javascript import evaluate_code
        elif language == 'java':
            from java import evaluate_code
        else:
    ...
    if file_path:
        try:
            ...
            if not unsafe_lines:
                ...
                for line, vuln_type, recommendation in unsafe_lines:
                    result_message += f"{Fore.RED}{line} - {vuln_type}\n{Fore.BLUE}Recommendation: {recommendation}{Style.RESET_ALL}\n"
...
def ml_menu():
    while True:
        ...
        print(f"{Fore.GREEN}*    Source Code Vulnerability Scanner  *{Style.RESET_ALL}")
        print(f"{Fore.GREEN}*****************************************{Style.RESET_ALL}\n")
